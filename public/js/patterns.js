...
// 1. FLOWING HILLS WITH PINE TREES
  ...
  for (let i = 0; i < layers; i++) {
    ...
    for (let x = 0; x <= w + geometryStep; x += geometryStep) {
      ...
      if (i < 2) {
        ...
        y = baseY + fbm(nx * mountainFreq + i * 4.5) * h * mountainAmplitude - (isDesktop ? h * 0.12 : h * 0.05);
      } else {
        const hillAmplitude = isDesktop ? 0.16 : 0.06;
        const hillFreq = isDesktop ? 2.5 : 1.5;
        y = baseY + fbm(nx * hillFreq + i * 2.1) * h * hillAmplitude - (isDesktop ? h * 0.01 : h * 0.02);
      }
...
export function drawPattern(ctx, w, h, pattern, palette, s, zoomLevel = 100, fitMode = 'crop') {
  ...
  switch(PATTERNS[pattern]) {
    ...
    default:                drawFlowingHills(ctx, w, h, palette, rng); break;
  }
